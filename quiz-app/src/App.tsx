...
/* --- CONFIGURATION --- */
const apiKey = ""; // Gemini API key bo'lsa shu yerga qo'ying

/* --- TYPES --- */
type View = "home" | "lessons" | "quiz" | "admin" | "login";
...
const MOCK_SECTIONS: Section[] = [
  ...
  {
    id: "2",
    name: "Oxford Word Skills",
    description: "Academic Vocabulary (B1-B2)",
    order: 2,
    questionCount: 1,
  },
];
...
const MOCK_QUESTIONS: Question[] = [
  {
    ...
    topic: "Unit 1",
    question: "Ambitious",
    choices: ["Mehnatsevar", "Oliyjanob", "Maqsad sari intiluvchan", "Dangasa"],
    correct_index: 2,
    explanation:
      "Ambitious - katta maqsadlari bor, o'ziga ishongan inson (Maqsad sari intiluvchan).",
    difficulty: "medium",
  ...
  },
  {
    ...
    choices: ["To decide", "To make", "To do", "To wonder"],
    correct_index: 0,
...
const Button = ({
  ...
  const variants: Record<ButtonVariant, string> = {
    ...
    secondary:
      "bg-white text-slate-700 border border-slate-200 hover:bg-slate-50 hover:border-slate-300 shadow-sm dark:bg-slate-800 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-700",
    danger:
      "bg-red-50 text-red-600 hover:bg-red-100 border border-transparent dark:bg-red-900/20 dark:text-red-400",
...
type BadgeColor = "blue" | "green" | "yellow" | "gray" | "red";
type BadgeProps = { children?: ReactNode; color?: BadgeColor; className?: string };
...
export default function App() {
  ...
  const startQuiz = (lessonName: string) => {
    ...
    const sectionName =
      sections.find((s) => s.id === activeSectionId)?.name ?? "Bo'lim";

    setActiveQuiz({
      sectionId: activeSectionId,
      sectionName: `${sectionName} - ${lessonName}`,
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans selection:bg-indigo-100 selection:text-indigo-900">
        ...
        <div className="max-w-5xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center space-x-2.5">
            ...
            <h1 className="text-xl font-bold tracking-tight">
              Vocab<span className="text-indigo-600 dark:text-indigo-400">Master</span>
            </h1>
          </div>
      ...
      <main className="max-w-5xl mx-auto px-4 py-12">
        <div className="text-center mb-16 space-y-4">
          <Badge className="mb-2" color="blue">
            So'z boyligini oshirish
          </Badge>
          <h2 className="text-4xl md:text-5xl font-extrabold text-slate-900 dark:text-white tracking-tight">
            Ingliz tili so'zlarini{" "}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-violet-600">
              yod oling
            </span>
          </h2>
          <p className="text-lg text-slate-600 dark:text-slate-400 max-w-2xl mx-auto leading-relaxed">
            Quyidagi to'plamlardan birini tanlang va so'zlarni tarjimasi bilan topish bo'yicha
            mashq qiling.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {sections.map((section) => {
            const qCount = questions.filter(
              (q) => q.section_id === section.id && q.status === "published"
            ).length;

            return (
              <Card
                key={section.id}
                className="group hover:shadow-xl hover:shadow-indigo-500/5 hover:-translate-y-1 transition-all duration-300 overflow-hidden border-slate-200/60 flex flex-col h-full"
              >
                <div className="p-6 flex-1 flex flex-col">
                  ...
                  <div className="mt-auto">
                    <Button className="w-full" onClick={() => openSection(section.id)}>
...
type LessonsViewProps = {
  ...
  questions: Question[];
  onBack: () => void;
  onStartLesson: (lessonName: string) => void;
};

function LessonsView({ sectionId, sections, questions, onBack, onStartLesson }: LessonsViewProps) {
  const section = useMemo(() => sections.find((s) => s.id === sectionId), [sections, sectionId]);

  const lessonList = useMemo(() => {
    const predefined = PREDEFINED_LESSONS[sectionId] ?? [];
              ...
              >
                ...
                <div className="z-10 flex items-center justify-between mt-2">
                  ...
                  {qCount > 0 && (
                    <Play
                      size={20}
                      className="text-indigo-600 opacity-0 group-hover:opacity-100 transition-opacity transform translate-x-2 group-hover:translate-x-0"
                      fill="currentColor"
                    />
                  )}
                </div>
      ...
      <Card className="w-full max-w-md p-8 animate-in zoom-in-95 duration-200">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-500 text-sm mt-2">
            Iltimos, kirish uchun ma'lumotlarni kiriting
          </p>
        </div>
        <div className="space-y-4">
          ...
          {/* Password hozir demo, ishlatilmaydi — lekin UI uchun qoldirdik */}
          <Button className="w-full mt-2" onClick={() => onLogin(email)}>
            Kirish
          </Button>
...
function QuizView({ quiz, setQuiz, onExit }: QuizViewProps) {
  const [isAnimating, setIsAnimating] = useState(false);

  const currentQ = quiz.questions[quiz.currentIndex];
  const isLast = quiz.currentIndex === quiz.questions.length - 1;
  const hasAnswered = Object.prototype.hasOwnProperty.call(quiz.answers, quiz.currentIndex);
  ...
  }, [hasAnswered, quiz.currentIndex, quiz.isFinished]);
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">
                        Javob:{" "}
                        <span className="font-semibold">{q.choices[q.correct_index]}</span>
                        {!isRight && userAns !== undefined && (
                          <span> (Siz: {q.choices[userAns as 0 | 1 | 2 | 3]})</span>
                        )}
                      </p>
                    </div>
                  </div>
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col items-center">
      <div className="w-full bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 sticky top-0 z-10">
        <div className="max-w-3xl mx-auto px-4 h-16 flex items-center justify-between">
          <Button variant="ghost" size="sm" onClick={onExit} icon={ArrowLeft}>
            Chiqish
          </Button>
      ...
      <div
        className={`flex-1 w-full max-w-3xl p-6 flex flex-col justify-center transition-opacity duration-200 ${
          isAnimating ? "opacity-50" : "opacity-100"
        }`}
      >
        <div className="mb-10 text-center">
          <div className="text-slate-500 text-sm uppercase tracking-wider mb-2 font-medium">
            Tarjimasini toping
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          {currentQ.choices.map((choice, idx) => {
            ...
            let cardClass =
              "hover:border-indigo-400 hover:bg-slate-50 dark:hover:bg-slate-800/50 cursor-pointer";
            let indicator = (
              <div className="w-8 h-8 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-500 font-bold flex items-center justify-center border border-slate-200 dark:border-slate-700 transition-colors">
                {["A", "B", "C", "D"][idx]}
              </div>
            );
            ...
            if (hasAnswered) {
              ...
              if (isCorrectChoice) {
                cardClass =
                  "bg-green-50 border-green-500 shadow-md shadow-green-100 dark:bg-green-900/20 dark:border-green-600 dark:shadow-none opacity-100 ring-1 ring-green-500";
                indicator = (
                ...
                cardClass =
                  "bg-red-50 border-red-500 dark:bg-red-900/20 dark:border-red-600 opacity-100";
                indicator = (
            ...
            return (
              <div
                ...
                className={`group relative p-4 rounded-xl border-2 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 transition-all duration-200 flex items-center gap-4 h-full ${cardClass}`}
              >
                {indicator}
                <span
        ...
        <div
          className={`transition-all duration-300 transform ${
            hasAnswered ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0 pointer-events-none"
          }`}
        >
          <div
            className={`p-6 rounded-2xl mb-6 flex gap-4 ${
              isCorrect
                ? "bg-green-100/50 dark:bg-green-900/20 text-green-900 dark:text-green-100"
                : "bg-red-100/50 dark:bg-red-900/20 text-red-900 dark:text-red-100"
            }`}
          >
...
function AdminDashboard({ sections, setSections, questions, setQuestions, onLogout }: AdminDashboardProps) {
  const [activeTab, setActiveTab] = useState<"sections" | "questions" | "import">("questions");
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex font-sans">
      ...
      <main className="flex-1 ml-64 p-8 overflow-y-auto min-h-screen">
        {activeTab === "sections" && <SectionsManager sections={sections} setSections={setSections} />}
        {activeTab === "questions" && (
...
const NavItem = ({ active, onClick, icon: Icon, label }: NavItemProps) => (
  <button
    type="button"
    onClick={onClick}
...
function SectionsManager({ sections, setSections }: SectionsManagerProps) {
  ...
  const handleSave = () => {
    ...
    if (editingId) {
      setSections((prev) =>
        prev.map((s) => (s.id === editingId ? { ...s, name: formData.name, description: formData.description } : s))
      );
    } else {
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        ...
        <Button onClick={() => setIsEditing(true)} icon={Plus}>
          Yangi Bo'lim
        </Button>
      </div>
        ...
        <Card className="mb-8 p-6 animate-in slide-in-from-top-4 border-indigo-200 dark:border-indigo-900">
          ...
          <div className="grid gap-4 mb-6">
            <div>
              ...
              <input
                className="w-full mt-1 p-2.5 border rounded-lg dark:bg-slate-900 dark:border-slate-700 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none"
                placeholder="Masalan: Essential Vocabulary"
                value={formData.name}
      ...
      <div className="grid gap-4">
        {sections.map((s) => (
          <Card
            key={s.id}
            className="p-5 flex justify-between items-center group hover:border-indigo-300 dark:hover:border-indigo-700 transition-colors"
          >
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 rounded-lg bg-slate-100 dark:bg-slate-700 flex items-center justify-center text-slate-500 font-bold">
                {s.order}
              </div>
              <div>
...
function QuestionsManager({ questions, setQuestions, sections }: QuestionsManagerProps) {
                ...
                <tr key={q.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                  <td className="px-6 py-4">
                    ...
                    <div className="text-xs text-slate-400 mt-1">
                      Javob: <span className="text-indigo-600 font-medium">{q.choices[q.correct_index]}</span>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    {q.status === "published" ? (
                      ...
                      <button
                        ...
                        className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-700 hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-300 cursor-pointer"
                      >
                        Qoralama
                      </button>
                    )}
...
function QuestionEditorModal({ question, sections, onClose, onSave }: QuestionEditorModalProps) {
  ...
  const handleChoiceChange = (index: 0 | 1 | 2 | 3, value: string) => {
  ...
  const handleSubmit = () => {
    ...
    if (formData.choices.some((c) => !c.trim())) return;
    onSave(formData);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
      <Card className="w-full max-w-2xl p-6 relative shadow-2xl overflow-y-auto max-h-[90vh]">
        ...
        <h2 className="text-xl font-bold mb-6 dark:text-white">
          {question.id ? "So'zni Tahrirlash" : "Yangi So'z"}
        </h2>
                ...
                return (
                  <div key={idx} className="flex items-center gap-2">
                    <input
                      ...
                      onChange={() => handleChange("correct_index", i)}
                      className="w-4 h-4 text-indigo-600"
                    />
                    <input
                      className={`flex-1 p-2 border rounded-lg dark:bg-slate-800 ${
                        formData.correct_index === i ? "border-green-500 ring-1 ring-green-500" : ""
                      }`}
                      value={choice}
...
type ImportExportManagerProps = {
  questions: Question[];
  setQuestions: React.Dispatch<React.SetStateAction<Question[]>>;
  sections: Section[];
};

function ImportExportManager({ questions, setQuestions, sections }: ImportExportManagerProps) {
  const [exportSectionId, setExportSectionId] = useState<string>("all");
  const [importSectionId, setImportSectionId] = useState<string>(sections[0]?.id || "");
  ...
  useEffect(() => {
    ...
    if (lessons.length > 0) setImportTopic(lessons[0]);
    else setImportTopic("");
  }, [importSectionId]);

  const exportJson = () => {
    let dataToExport: Question[] = questions;
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        if (typeof result !== "string") throw new Error("Fayl o'qilmadi");

        const json: unknown = JSON.parse(result);
        if (!Array.isArray(json)) throw new Error("Fayl JSON array bo'lishi kerak ([...]).");

        // basic validation + convert
        const parsed = json.map((qAny) => qAny as Partial<Question>);

        const isValid = parsed.every((q) => q.question && q.choices && q.correct_index !== undefined);
        if (!isValid) throw new Error("JSON formatida kerakli maydonlar (question, choices...) yetishmayapti.");

        const newQuestions: Question[] = parsed.map((q) => {
          const rawChoices = (q.choices ?? ["", "", "", ""]) as string[];
          const fixedChoices: [string, string, string, string] = [
            rawChoices[0] ?? "",
            rawChoices[1] ?? "",
            rawChoices[2] ?? "",
            rawChoices[3] ?? "",
          ...
          const idx = (q.correct_index ?? 0) as number;
          const safeIndex: 0 | 1 | 2 | 3 = (idx === 1 || idx === 2 || idx === 3 ? idx : 0);
          ...
          return {
            ...
            status: (q.status as QuestionStatus) ?? "draft",
            created_at: new Date().toISOString(),
            created_by: "manual",
          };
        });
        ...
        setNotification({
          type: "success",
          message: `Muvaffaqiyatli! ${newQuestions.length} ta so'z "${sectionName}"ning "${importTopic}" darsiga qo'shildi.`,
        });
      } catch (err) {
    ...
    <div className="max-w-4xl mx-auto">
      ...
      {notification && (
        ...
        >
          ...
          <button type="button" className="ml-auto opacity-50 hover:opacity-100" onClick={() => setNotification(null)}>
            <X size={18} />
          </button>
        </div>
      )}
        ...
        <Card className="p-8 text-center border-dashed border-2 border-slate-300 dark:border-slate-700 shadow-none hover:border-indigo-500 transition-colors flex flex-col h-full">
          ...
          <div className="space-y-3 mb-6 w-full text-left">
            ...
            <div>
              ...
              >
                {(PREDEFINED_LESSONS[importSectionId] ?? []).map((lesson) => (
...
function AiGeneratorModal({ sections, onClose, onGenerate }: AiGeneratorModalProps) {
...
5. "explanation" must be in Uzbek.
  ...
  "questions": [
    {
      ...
      if (!apiKey) {
        throw new Error("API key kiritilmagan (apiKey bo'sh).");
      }
      ...
      if (!text) throw new Error("AI javobi bo'sh");

      const parsed = JSON.parse(text) as { questions?: Array<any> };
      ...
      const newQuestions: Question[] = parsed.questions.map((q: any) => {
        const rawChoices = Array.isArray(q.choices) ? (q.choices as string[]) : ["", "", "", ""];
        const fixedChoices: [string, string, string, string] = [
          String(rawChoices[0] ?? ""),
          String(rawChoices[1] ?? ""),
          String(rawChoices[2] ?? ""),
      ...
      setError("Xatolik yuz berdi: " + msg);
    } finally {
      setLoading(false);
    }
  };
